...
namespace UnLua
{
    ...
    struct TArgumentIntelliSense<T1, T2...>
    {
        ...
        {
            FString TypeName = TTypeIntelliSense<
                typename std::conditional<
                    TIsPointer<T1>::Value,
                    typename TDecay<typename TRemovePointer<T1>::Type>::Type*,
                    typename TDecay<T1>::Type
                >::type
            >::GetName();
    ...
    void GenerateArgsIntelliSense(FString &Buffer, FString &ArgList)
    {
        ...
        // return 
        FString ReturnTypeName = TTypeIntelliSense<
            typename std::conditional<
                TIsPointer<RetType>::Value,
                typename TDecay<typename TRemovePointer<RetType>::Type>::Type*,
                typename TDecay<RetType>::Type
            >::type
        >::GetName();
    ...
    */
    ...
    TExportedClass<bIsReflected, ClassType, CtorArgType...>::TExportedClass(const char *InName, const char *InSuperClassName) : FExportedClassBase(InName, InSuperClassName)
    {
    ...
    {
        AddConstructor(typename std::conditional<
            TIsConstructible<ClassType, CtorArgType...>::Value,
            FTrue,
            FFalse
        >::type());

        AddDestructor(typename std::conditional<
            TAnd<
                TIsDestructible<ClassType>,
                TNot<TStaticBool<std::is_trivially_destructible_v<ClassType>>>
            >::Value,
            FFalse,
            FTrue
        >::type());
