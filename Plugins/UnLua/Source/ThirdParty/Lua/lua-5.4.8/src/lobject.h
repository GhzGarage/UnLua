...
typedef struct LuaString {
  ...
  union {
    ...
    struct LuaString *hnext;  /* linked list for hash table */
  } u;
  char contents[1];
} LuaString;
...
/*
** Get the actual string (array of bytes) from a 'LuaString'. (Generic
...
/* get string length from 'LuaString *s' */
#define tsslen(s)  \
